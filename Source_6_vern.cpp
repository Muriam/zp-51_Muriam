#include <iostream>
#include <conio.h>
#include <cstring> //дл€ ф-ции strcmp
#define N 8
...
bool ReadBase(dataBase *p, int* indArr, int* indArr2) //указатель на переменную с типом структуры, указатель на индексные массивы
{
	...
	{
		fread(p[i].name, sizeof(char), nameSize, base);  //(где 1 - количество €чеек пам€ти, 32 - количество символов в €чейках) 
		...
		p[i].date[dateSize] = 0;
		indArr[i] = i;//—оздание индексного массива
		indArr2[i] = i; //—оздание индексного массива2
	}
	fclose(base);
	return 0;
}
...
int bubbleSortName(dataBase *db, int* indArr)
{
	...
	for (int i = 0; i<N - 1; i++)
	{
		for (int j = 0; j<N - 1 - i; j++)
			...
			{
				// ћен€ем элементы местами. Ёлементы массива db фактически не перемещаютс€, но мы мен€ем их пор€док в индексном массиве.
				temp = indArr[j];
				indArr[j] = indArr[j + 1];
				indArr[j + 1] = temp;
			}
...
int bubbleSortSum(dataBase *db, int* indArr)
{
	...
	{
		for (int j = 0; j<N-1; j++)
			if (db[indArr[j]].sumContribution > db[indArr[j + 1]].sumContribution)   //—равниваем элементы массива, через индексный массив
